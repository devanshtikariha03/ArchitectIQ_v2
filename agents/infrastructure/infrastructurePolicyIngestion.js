...
function slug(value) {
  return String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'infrastructure-policy';
}
...
function policyFreshness({ effectiveDate, reviewBy, owner, version }) {
  ...
  if (!effectiveDate || effectiveDate === 'not stated') {
    ...
    } else if (Math.floor((now - effective) / 86400000) > 365) {
      status = 'stale';
      notes.push('Policy effective date is more than 365 days old.');
    }
  }
...
function parseMarkdownPolicy(filePath, kind) {
  ...
  const tags = fieldValue(content, 'Tags').split(',').map(item => item.trim()).filter(Boolean);
...
function readMarkdownPolicies(dirPath, kind) {
  ...
  return fs.readdirSync(dirPath).filter(file => file.toLowerCase().endsWith('.md')).map(file => path.join(dirPath, file)).map(filePath => {
    try {
      ...
      return {
        ...
        validation_needed: [`Fix Infrastructure policy markdown parse error in ${path.relative(process.cwd(), filePath)}.`],
        citations: [],
...
function loadInfrastructurePolicyDocuments() {
  return [
    ...readMarkdownPolicies(KNOWLEDGE_DIR, 'architectiq-infrastructure-knowledge'),
    ...readMarkdownPolicies(CUSTOMER_POLICY_DIR, 'customer-infrastructure-policy-pack'),
  ];
}
