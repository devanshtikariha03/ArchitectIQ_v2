...
function slug(value) {
  return String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'finops-policy';
}
  ...
  return fs.readdirSync(dirPath).filter(file => file.toLowerCase().endsWith('.md')).map(file => path.join(dirPath, file)).map(filePath => {
    try {
      ...
      return {
        ...
        validation_needed: [`Fix FinOps policy markdown parse error in ${path.relative(process.cwd(), filePath)}.`],
        citations: [],
...
function loadFinOpsPolicyDocuments() {
  return [
    ...readMarkdownPolicies(KNOWLEDGE_DIR, 'architectiq-finops-knowledge'),
    ...readMarkdownPolicies(CUSTOMER_POLICY_DIR, 'customer-finops-policy-pack'),
  ];
}
