...
function parseMarkdownPolicy(filePath, kind) {
  ...
  return {
    ...
    compliance_mappings: linesForSection(content, 'Compliance Mappings'),
    ...
    .map(filePath => {
      try {
        ...
        return {
          ...
          validation_needed: [`Fix security policy markdown parse error in ${path.relative(process.cwd(), filePath)}.`],
          compliance_mappings: [],
          citations: [],
...
function loadSecurityPolicyDocuments() {
  return [
    ...readMarkdownPolicies(KNOWLEDGE_DIR, 'architectiq-security-knowledge'),
    ...readMarkdownPolicies(CUSTOMER_POLICY_DIR, 'customer-security-policy-pack'),
  ];
}
