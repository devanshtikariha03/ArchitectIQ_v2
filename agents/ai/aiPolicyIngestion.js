...
function slug(value) {
  return String(value || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'ai-policy';
}
  ...
  return fs.readdirSync(dirPath).filter(file => file.toLowerCase().endsWith('.md')).map(file => path.join(dirPath, file)).map(filePath => {
    try {
      ...
      return {
        ...
        validation_needed: [`Fix AI policy markdown parse error in ${path.relative(process.cwd(), filePath)}.`],
        citations: [],
...
function loadAiPolicyDocuments() {
  return [
    ...readMarkdownPolicies(KNOWLEDGE_DIR, 'architectiq-ai-knowledge'),
    ...readMarkdownPolicies(CUSTOMER_POLICY_DIR, 'customer-ai-policy-pack'),
  ];
}
