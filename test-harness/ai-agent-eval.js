...
const { runAiAgentGraph } = require('../agents/ai/aiAgentGraph');
const { AI_EVAL_CASES, scoreAiOutput } = require('../agents/ai/aiEvalCases');

const OUT_DIR = path.join(__dirname, 'results');
if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

async function runCase(testCase) {
  const output = await runAiAgentGraph({
...
(async () => {
  const results = [];
  for (const testCase of AI_EVAL_CASES) {
  ...
  const outPath = path.join(OUT_DIR, `ai-agent-eval-${ts}.json`);
  fs.writeFileSync(outPath, JSON.stringify({ ran: new Date().toISOString(), results }, null, 2));
  console.log(`Results saved: ${outPath}`);

  if (failures.length) {
    console.error(`AI eval failed: ${failures.length}/${results.length} case(s) below threshold.`);
    process.exit(1);
  }
})();
