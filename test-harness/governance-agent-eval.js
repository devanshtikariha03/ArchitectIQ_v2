...
const { runGovernanceAgentGraph } = require('../agents/governance/governanceAgentGraph');
const { GOVERNANCE_EVAL_CASES, scoreGovernanceOutput } = require('../agents/governance/governanceEvalCases');

const OUT_DIR = path.join(__dirname, 'results');
if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

async function runCase(testCase) {
  const output = await runGovernanceAgentGraph({
  ...
  return {
    ...scoreGovernanceOutput(output, testCase),
...
(async () => {
  const results = [];
  for (const testCase of GOVERNANCE_EVAL_CASES) {
  ...
  const outPath = path.join(OUT_DIR, `governance-agent-eval-${ts}.json`);
  fs.writeFileSync(outPath, JSON.stringify({ ran: new Date().toISOString(), results }, null, 2));
  console.log(`Results saved: ${outPath}`);

  if (failures.length) {
    console.error(`Governance eval failed: ${failures.length}/${results.length} case(s) below threshold.`);
    process.exit(1);
  }
})();
