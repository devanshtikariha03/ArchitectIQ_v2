...
const { runUiAgentGraph } = require('../agents/ui/uiAgentGraph');
const { UI_EVAL_CASES, scoreUiOutput } = require('../agents/ui/uiEvalCases');

const OUT_DIR = path.join(__dirname, 'results');
if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

async function runCase(testCase) {
  const output = await runUiAgentGraph({
...
(async () => {
  const results = [];
  for (const testCase of UI_EVAL_CASES) {
  ...
  const outPath = path.join(OUT_DIR, `ui-agent-eval-${ts}.json`);
  fs.writeFileSync(outPath, JSON.stringify({ ran: new Date().toISOString(), results }, null, 2));
  console.log(`Results saved: ${outPath}`);

  if (failures.length) {
    console.error(`UI eval failed: ${failures.length}/${results.length} case(s) below threshold.`);
    process.exit(1);
  }
})();
