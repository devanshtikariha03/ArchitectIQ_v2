...
const { runStorageAgentGraph } = require('../agents/storage/storageAgentGraph');
const { STORAGE_EVAL_CASES, scoreStorageOutput } = require('../agents/storage/storageEvalCases');

const OUT_DIR = path.join(__dirname, 'results');
if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

async function runCase(testCase) {
  const output = await runStorageAgentGraph({
  ...
  return {
    ...scoreStorageOutput(output, testCase),
...
(async () => {
  const results = [];
  for (const testCase of STORAGE_EVAL_CASES) {
  ...
  const outPath = path.join(OUT_DIR, `storage-agent-eval-${ts}.json`);
  fs.writeFileSync(outPath, JSON.stringify({ ran: new Date().toISOString(), results }, null, 2));
  console.log(`Results saved: ${outPath}`);

  if (failures.length) {
    console.error(`Storage eval failed: ${failures.length}/${results.length} case(s) below threshold.`);
    process.exit(1);
  }
})();
